...
int fcfs(const vector<Process> &processes, int time)
{
    int indx = -1;
    for (int i = 0; i < processes.size(); i++)
    {
        if (processes[i].remainingBT > 0 && processes[i].AT <= time)
        {
            if (indx == -1 || processes[i].AT < processes[indx].AT || (processes[i].AT == processes[indx].AT && processes[i].name < processes[indx].name))
            {
                indx = i;
            }
        }
...
void srjf(vector<Process> &processes, vector<pair<int, string>> &ganttChart, double &totalTT, double &totalWT, int &finalTime)
{
    ...
    while (completed < n)
    {
        ...
        if (idx == -1) {
            ...
            // Tie-breaking for processes with the same remaining burst time
            vector<int> tieBreakCandidates;
            for (int i = 0; i < n; i++) {
                if (processes[i].remainingBT > 0 && processes[i].AT <= time && processes[i].remainingBT == minRemainingBT) {
                    tieBreakCandidates.push_back(i);
                }
            }
            if (tieBreakCandidates.size() > 1) {
                idx = fcfs(processes, time);
            }
